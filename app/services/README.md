...
| `knowledge_client.py` | `knowledge_service_client` | 访问独立 knowledge-service 的唯一适配层（复用持久连接、Token/操作人/Trace 透传） |
...
4. **`knowledge_client.py`** — 看 knowledge-service 代理：理解 `X-KB-Service-Token`、操作人透传、错误收敛，以及持久连接复用（避免每次新建 AsyncClient 的秒级开销）。
