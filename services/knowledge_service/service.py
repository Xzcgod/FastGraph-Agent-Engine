...
async def search(session: AsyncSession, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if kb_ids:
        searchable_statement = searchable_statement.where(KnowledgeBase.id.in_([str(kb_id) for kb_id in kb_ids]))
    namespace = payload.get("namespace")
    if namespace:
        searchable_statement = searchable_statement.where(KnowledgeBase.namespace == str(namespace))
    ...
    if kb_ids:
        statement = statement.where(KnowledgeBase.id.in_([str(kb_id) for kb_id in kb_ids]))
    if namespace:
        statement = statement.where(KnowledgeBase.namespace == str(namespace))

    rows = (await session.execute(statement)).all()
    items = []
    for chunk, document, kb, raw_distance in rows:
        if not metadata_matches(metadata_filter, document.metadata_json or {}, chunk.metadata_json or {}):
            continue
    ...
    reranked = await rerank_items(query, items)
    return {"items": reranked[:top_k], "total": min(len(reranked), top_k)}
